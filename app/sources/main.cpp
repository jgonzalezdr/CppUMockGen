...
static bool ParseInputFile( const std::string &input, const Config &config, bool interpretAsCpp,
...
int main( int argc, char* argv[] )
{
    ...
    try
    {
        ...
        if( ParseInputFile( inputFileName, config, interpretAsCpp, options["include-path"].as<std::vector<std::string>>(), output ) )
        {
            if( options.count( "output" ) )
            {
